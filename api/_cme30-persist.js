...
/**
 * NervaFX Currency Movement Engine (30M) — persistence (isolated).
 * Own tables cme30_currency_movement_values / cme30_pair_structure. Never throws.
 */

async function persistCme30(sb, scan) {
  const rows = [];
  const at = scan.generatedAt;
  const version = scan.engineVersion || 'v1';
  const configVersion = scan.configurationVersion || 'structure_30m_v1';
  ...
  try {
    if (rows.length) {
      const { error } = await sb.from('cme30_currency_movement_values').upsert(rows, { onConflict: 'evaluated_at,window_name,currency,engine_version' });
  ...
  try {
    ...
    if (pairs.length) {
      const { error } = await sb.from('cme30_pair_structure').upsert(pairs, { onConflict: 'evaluated_at,pair,configuration_version' });
      if (error) throw error;
