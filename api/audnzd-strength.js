...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (const hour of hours) {
      ...
      if (audRank === 1 && audVal > 0.015) audSignal = 'STRONGEST';
      else if (audRank === 8 && audVal < -0.015) audSignal = 'WEAKEST';
      ...
      timeline.push({
        time: hour,
        ranking: ranked,
        aud: { rank: audRank, value: audVal, signal: audSignal, trades: audTrades },
        nzd: { rank: nzdRank, value: nzdVal, signal: nzdSignal, trades: nzdTrades },
      });
    }
