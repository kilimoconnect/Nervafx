...
// Both of the last two closed H1 candles closed in the trade direction
// (buy: close > open on each; sell: close < open on each).
function twoH1SameDir(h1ohlc, dirSign) {
  if (!h1ohlc || h1ohlc.length < 2 || !dirSign) return false;
  const a = h1ohlc[h1ohlc.length - 1], b = h1ohlc[h1ohlc.length - 2];
  ...
  try {
    ...
    for (let b = 0; b < PAIRS.length; b += 7) {
      ...
      const rows = await Promise.all(batch.map(async inst => {
        let dom, mid, trig, domOHLC, m15ohlc, h1ohlc;
        if (M.synth) {
          const [m15o, m5c, h1o] = await Promise.all([
            fetchOHLC(sb, inst, 'M15', 500, untilFor('M15')),
            fetchCloses(sb, inst, 'M5', 300, untilFor('M5')),
            fetchOHLC(sb, inst, 'H1', 50, untilFor('H1')),
          ]);
          m15ohlc = m15o;
          h1ohlc = h1o;
          ...
          const [domRaw, midC, trigC, m15extra, h1extra] = await Promise.all([
            ...
            M.dom === 'H1' ? Promise.resolve(null) : fetchOHLC(sb, inst, 'H1', 50, untilFor('H1')),
          ]);
          m15ohlc = M.dom === 'M15' ? domRaw : m15extra;
          h1ohlc = M.dom === 'H1' ? domRaw : h1extra;
          domOHLC = domRaw.slice(-10);
          dom = snap(domRaw.map(c => c.close), domRaw.slice(-40));
          mid = snap(midC, null);
          trig = snap(trigC, null);
        }
        return { inst, dom, mid, trig, domOHLC, ha15: heikinAshi(m15ohlc), h1ohlc };
      }));
    ...
    for (const inst of PAIRS) {
      ...
      // Require the last two closed H1 candles to both close in the trade direction.
      const h1TwoDir = twoH1SameDir(P.h1ohlc, dir);
      ...
      pairs.push({
        ...
        direction: dir > 0 ? 'BUY' : 'SELL', state, score, broke, confirmAligned, h1TwoDir,
