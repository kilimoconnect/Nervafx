...
/**
 * GET /api/currency-strength-m5-ema
 *
 * Per-currency strength derived from M5 EMA20 / EMA50 / close on all 28
 * pairs. Same alignment maths as /api/currency-strength-h1-ema; only the
 * candle timeframe changes. The M5 series lands every 5 minutes courtesy
 * of the sync cron (now firing at 1,6,11,16... past the hour).
 *
...
async function fetchLastM5(sb, inst, limit = 80) {
  ...
  try {
    ...
    for (let b = 0; b < PAIRS.length; b += 7) {
      ...
      const rows = await Promise.all(batch.map(async inst => {
        const candles = await fetchLastM5(sb, inst, 80);
