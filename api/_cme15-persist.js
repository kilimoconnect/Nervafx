...
/**
 * NervaFX Currency Movement Engine (15M twin) — persistence (isolated).
 *
 * Writes to its OWN tables (cme15_currency_movement_values / cme15_pair_structure)
 * — never the H1 engine's tables. Idempotent, never throws; historical
 * (read-only) requests skip it. Mirrors _cme-persist.js.
 */

async function persistCme15(sb, scan) {
  const rows = [];
  const at = scan.generatedAt;
  const version = scan.engineVersion || 'v1';
  const configVersion = scan.configurationVersion || 'structure_15m_v1';
  ...
  try {
    if (rows.length) {
      const { error } = await sb.from('cme15_currency_movement_values')
  ...
  // Pair-level BOS (from the current M15 structure snapshot).
  try {
    ...
    if (pairs.length) {
      const { error } = await sb.from('cme15_pair_structure')
