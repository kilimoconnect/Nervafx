...
function buildTimeline(co, d1, h4, h1) {
  ...
  push(e.bosTime, 'H1', 'BOS', 'H1 BOS beyond the pullback swing');
...
function buildScanCard(input) {
  ...
  const bosConfirmed = !!(h1 && h1.bosConfirmed);
  // Stage / ranking. A confirmed H1 BOS (close beyond the pullback swing) is the
  // trigger; from there the setup waits for a retracement fill, then becomes a
  // live signal. No sweep / rejection step exists.
  ...
  if (h4.state === H4_STATE.PULLBACK_ACTIVE) { stage = 'H4_PULLBACK'; rank = 3; }   // aligned pullback, waiting for H1 BOS
  if (h4.state === H4_STATE.INVALIDATED || h4.state === H4_STATE.EXPIRED) { stage = 'INVALIDATED'; rank = -1; }
  if (bosConfirmed) {
    if (h1.state === H1_STATE.ACTIVE) { stage = 'SIGNAL'; rank = 6; }              // filled & live
    else if (h1.state === H1_STATE.ENTRY_PENDING) { stage = 'RETEST_PENDING'; rank = 5; } // BOS done, awaiting retracement
    else if (h1.rejection === 'ENTRY_MISSED') { stage = 'MISSED_ENTRY'; rank = -1; }
    else { stage = 'H1_BOS'; rank = 4; }                                           // BOS confirmed but candidate not viable / completed
  }
  const qualifies = signal != null;                                               // an actionable BUY/SELL candidate
  ...
  return {
    ...
    h1State: h1 ? h1.state : null, h1Status: h1 ? h1.status : null, bosConfirmed,
    stage, rank, qualifies,
    signal,
