...
module.exports = async function handler(req, res) {
  ...
  try {
      ...
      for (let i = triggerIdx + 1; i < m15s.length; i++) {
        ...
        if (c.close < ref.high && c.close > ref.low) {
          ...
          timeline.push({
            ...
            event: 'Close back inside prev H4 range',
            m15: {
    ...
    // Sort: qualified first; then by prev-H4 break strength (refBreakPips desc);
    // then by trigger/break time asc; finally by triggerBreakPips desc.
    pairs.sort((a, b) => {
