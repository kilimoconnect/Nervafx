...
function groupStructure(strong, weak) {
  ...
  if (s >= 6 && w <= 2) return { label: s + 'v' + w, imbalance: true };
  if (s >= 5 && w <= 3) return { label: s + 'v' + w, imbalance: true };
  if (w >= 5 && s <= 3) return { label: s + 'v' + w, imbalance: true };
  if (w >= 6 && s <= 2) return { label: s + 'v' + w, imbalance: true };
  if (s === 7 && w <= 1) return { label: s + 'v' + w, imbalance: true };
  if (w === 7 && s <= 1) return { label: s + 'v' + w, imbalance: true };
  if (s === 4 && w === 4) return { label: '4v4', imbalance: false };
  return { label: s + 'v' + w, imbalance: s !== w && Math.abs(s - w) >= 2 };
}
