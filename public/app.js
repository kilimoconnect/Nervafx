...
function renderJrnCsigSection(e) {
  ...
  function colHtml(list, side) {
    ...
    return `
      <div class="cs-sig-col ${side}">
        <div class="cs-sig-col-title">${isStrong ? '💪 Strong' : '🔻 Weak'}</div>
        <div class="cs-sig-head"><span>CCY</span><span>Combined</span><span>2H</span><span>4H</span><span>6H</span><span>12H</span><span>Daily</span></div>
        ${list.map(c => `
  ...
  const headerContent = `
    <div class="jrn-csig-avail-header">
      ...
      <span class="jrn-csig-criteria">combined > ±${CS_THRESHOLD.toFixed(5)} & 2H confirms</span>
      ${expansionNote}
...
function _setupAnalysis(s) {
  ...
  // ── 3. Spread magnitude ──────────────────────────────────────────────
  if (sp6h >= 0.002)       parts.push(`6H spread strong (${(sp6h * 10000).toFixed(0)} pips) — strong quality setup.`);
  ...
  if (isPB && sessLabel === 'LONDON' && decentSpread)
    ...
    parts.push('⟶ PATIENT — base forming with session support. Wait for 2H re-expansion.');
    ...
    return `
      <div class="live-card ${dir}">
        ...
        <div class="signal-tf-row">
          ...
          <span class="tf-item ${ta.h3}">2H ${ta.h3||'→'}</span>
  ...
  return `
    <div class="wl-card ${dir}${isEntry ? ' wl-entry' : ''}">
      ...
      <div class="wl-tf">
        ...
        <span class="tfa ${ta.h3}">2H ${ta.h3||'→'}</span>
        <span class="sb-behavior ${s.spread_behavior}">${clean(s.spread_behavior||'')}</span>
...
function renderCurrencySignals(data) {
  ...
  const scored = currencies.map(c => {
    ...
    return {
      cur: c.currency,
      combined: (vm15 + v3 + v6) / 3,  // M15+2H+6H
      c2: (vm15 + v3) / 2,             // M15+2H
    ...
    return `
      <div class="cs-sig-col ${side}">
        ...
        <div class="cs-sig-head">
          <span>CCY</span><span class="plan-pro-only">M15+2H</span><span class="plan-pro-only">M15+2H+6H</span><span class="plan-pro-only">M15</span><span>2H</span><span>4H</span><span>6H</span><span>12H</span><span>Daily</span>
        </div>
        ${list.map(c => `
          <div class="cs-sig-row">
            <span class="cs-sig-cur">${c.cur}</span>
...
function buildChart(data, tf) {
  ...
  const getVal = (c, t) => {
    ...
    if (t === 'c2')  return (vm15() + v3()) / 2;           // M15 + 2H
    if (t === 'c3')  return (vm15() + v3() + v6()) / 3;    // M15 + 2H + 6H
    if (t === 'combined') {
      const v12 = parseFloat(c.smooth_12h ?? c.normalized_12h ?? 0);
      return (v3() + v6() + v12) / 3;
    }
...
function renderSpreads(data) {
  ...
  if (!el) return;
  // Use server-sorted order (weighted: 40% 12H + 35% 6H + 15% 2H + 10% accel)
...
// ─── Flow Performance (session strength pairs · M15 & 2H) ───────────────────
...
async function renderFlowPerformance() {
  ...
  try {
    // Same API calls the dedicated pages use, so the dashboard shows identical fresh data
    const ts = Date.now(); // cache-bust the API response
    const [h1Data, m15Data] = await Promise.all([
      api(`/api/structure-break?hours=24&t=${ts}`),
      api(`/api/m15-quality?hours=24&t=${ts}`),
    ]);

    // Timeline is newest-first; take the 2 most recent snapshots
...
// Shows pairs with currency spread ≥ 40 pips (derived from live currency strength)
...
async function renderEnergySignals(data) { return; // Signals tab removed
  ...
  if (pairsEl && !isWeekend) {
    ...
    if (!activePairs.length) {
      ...
      // 40% spread alignment (2H/6H/12H weighted 45/35/20, signed in trade dir)
...
function renderPairAnalysis(data, container) {
    ...
    metricsHtml = `
      <div style="margin-bottom:16px;padding:14px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:10px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;text-align:center">
          ...
          <div>
            <div class="pa-stat-label">2H ${dot(h3Aligned)}</div>
  ...
  if (pair && priceStats) {
    ...
    if (aligned === 3) parts.push(`All timeframes (M15, 2H, 6H) confirm ${dirWord} pressure — full alignment.`);
...
async function renderCompressionBreakout(data) {
  ...
  // Filter structures to only pairs still in active signal pairs (2H spread ≥ 30p)
...
function _renderAnDashCard(sig) {
  ...
  html += '<div class="an-dash-card-body">';

  const ccys = [];
  if (sig.aud.signal) ccys.push({ ccy: 'AUD', ...sig.aud });
  if (sig.nzd.signal) ccys.push({ ccy: 'NZD', ...sig.nzd });

  for (const c of ccys) {
    const badgeCls = c.signal === 'STRONGEST' ? 'an-dash-strong' : 'an-dash-weak';
    html += `<div class="an-dash-ccy-row">`;
    html += `<span class="an-dash-ccy-badge ${badgeCls}">${c.ccy} ${c.signal === 'STRONGEST' ? '▲' : '▼'}</span>`;
    html += `<span class="an-dash-rank">#${c.rank}</span>`;
    html += `<span class="an-dash-val ${_anValCls(c.value)}">${_anFmtVal(c.value)}</span>`;
    html += `</div>`;

    if (c.trades && c.trades.length) {
      html += '<div class="an-dash-trades">';
      for (const t of c.trades) {
        const dirCls = t.direction === 'BUY' ? 'an-dash-buy' : 'an-dash-sell';
        const scoreTag = t.score != null ? `<span class="an-dash-score">${t.score}</span>` : '';
        html += `<div class="an-dash-trade">`;
        html += `<span class="an-dash-dir ${dirCls}">${t.direction}</span>`;
        html += `<span class="an-dash-pair">${t.pair}</span>`;
        html += scoreTag;
        html += `</div>`;
      }
      html += '</div>';
...
function _renderQpCard(snap, type) {
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    html += `<div class="qp-pair${p.trending ? ' qp-trending' : ''}">
      ...
      ${p.dayHigh && p.direction === 'BULLISH' ? '<span class="qp-dayhl">⬆ DAY HIGH</span>' : ''}
      ${p.dayLow && p.direction === 'BEARISH' ? '<span class="qp-dayhl">⬇ DAY LOW</span>' : ''}
...
async function fetchInlineM15EnergyBars() {
  ...
  if (!el) return;
  try {
    const data = await api('/api/m15-energy?days=5');
...
function _renderInlineM15Bars(container, bars) {
  ...
  for (const date of dates) {
    ...
    if (volPoints.length > 1) {
      ...
      html += `<div style="position:absolute;left:${pk.x}%;top:0;transform:translateX(-50%);pointer-events:none;z-index:3;white-space:nowrap">
...
const METRIC_CHART_CONFIG = {
  ...
  energy: {
    ...
    unit: '', decimals: 0, v2Threshold: 60, m15Threshold: 60,
    thresholds: [
    ...
    guide: [
      ...
      '<strong>Pink bars</strong> = full confluence — energy ≥ 60 AND Dispersion, Tradability, Movement, Breadth and Agreement were ALL green in that same hour. The highest-conviction conditions.',
  ...
  de: {
    ...
    thresholds: [
      ...
      { min: 35, color: '#f59e0b', label: 'Mixed' },
...
function renderJrnStrengthSection(cs) {
  ...
  const tfs = [
    { label: '2H',  keys: ['smooth_2h',  'normalized_2h']  },
    { label: '6H',  keys: ['smooth_6h',  'normalized_6h']  },
    { label: '12H', keys: ['smooth_12h', 'normalized_12h'] },
  ];

  // Build combined (equal-weight average of 2H + 6H + 12H)
