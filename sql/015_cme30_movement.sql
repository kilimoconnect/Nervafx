-- NervaFX Currency Movement Engine — 30M variant persistence (isolated, additive).
...
-- Primary timeframe M30, micro M15, BOS lookback 10. All timestamps UTC.

create table if not exists cme30_currency_movement_values (
  id                            bigserial primary key,
  engine_version                text not null default 'v1',
  configuration_version         text,
  evaluated_at                  timestamptz not null,
  window_name                   text not null,     -- M30 | M15 | REFERENCE_SESSION | ASIA_TO_DATE | LONDON_TO_DATE | DAY_TO_DATE
  ...
  micro_state                   text,
  -- BOS structure layer (structure_30m_v1)
...
create index if not exists idx_cme30_eval_window on cme30_currency_movement_values (evaluated_at, window_name);
create index if not exists idx_cme30_currency on cme30_currency_movement_values (currency, window_name);

-- Pair-level Break of Structure (M15 primary).
create table if not exists cme30_pair_structure (
  id                    bigserial primary key,
  engine_version        text not null default 'v1',
  configuration_version text not null default 'structure_30m_v1',
  evaluated_at          timestamptz not null,
  pair                  text not null,
  base_currency         text,
  quote_currency        text,
...
create index if not exists idx_cme30_pair_struct_eval on cme30_pair_structure (evaluated_at);
create index if not exists idx_cme30_pair_struct_pair on cme30_pair_structure (pair);
