-- NervaFX Currency Movement Engine — 5M variant persistence (isolated, additive).
...
-- Primary timeframe M5, no micro, BOS lookback 60. All timestamps UTC.

create table if not exists cme05_currency_movement_values (
  id                            bigserial primary key,
  engine_version                text not null default 'v1',
  configuration_version         text,
  evaluated_at                  timestamptz not null,
  window_name                   text not null,     -- M5 | REFERENCE_SESSION | ASIA_TO_DATE | LONDON_TO_DATE | DAY_TO_DATE
  ...
  micro_state                   text,
  -- BOS structure layer (structure_5m_v1)
...
create index if not exists idx_cme05_eval_window on cme05_currency_movement_values (evaluated_at, window_name);
create index if not exists idx_cme05_currency on cme05_currency_movement_values (currency, window_name);

-- Pair-level Break of Structure (M15 primary).
create table if not exists cme05_pair_structure (
  id                    bigserial primary key,
  engine_version        text not null default 'v1',
  configuration_version text not null default 'structure_5m_v1',
  evaluated_at          timestamptz not null,
  pair                  text not null,
  base_currency         text,
  quote_currency        text,
...
create index if not exists idx_cme05_pair_struct_eval on cme05_pair_structure (evaluated_at);
create index if not exists idx_cme05_pair_struct_pair on cme05_pair_structure (pair);
